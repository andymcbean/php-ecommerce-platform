        ...
        if(isset($_POST['submit']))
            {
                if(empty($_POST['title']))
                    {
                        echo "<br><div class='container'><div class='alert alert-danger'> Title required. Please <a style='color: red;' href='../admin/newsletter'> try again.</a></div></div>";
                        exit();
                    }
                else
                    {
                ...
                if(empty($_POST['post']))
                    {
                        echo "<br><div class='container'><div class='alert alert-danger'> Article required. Please <a style='color: red;' href='../admin/newsletter'> try again.</a></div></div>";
                        exit();
                    }
                else
                    {
                        ...
                        try
                            {
                                $statement->execute();
                                //echo newsletter();
                                $success = "<br><div class='container'><div class='alert alert-success'> Newsletter added to database and sent to subscribers <a href='composed-newsletters'>View before sending</a></div></div>";
                            }
...
<div class="hero-image">
  <div class="hero-text">
    <h1 style="font-size:50px; color: #fff" class="nav-icon">Edit Newsletter</h1>
  </div>
</div>
            ...
            <form name="form" action="<?php $_SERVER["PHP_SELF"] ?>" method="post" id="details_contact" enctype="multipart/form-data">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label>Upload Image(s) to be intserted into this newsletter. Try and keep image file size as small as possible.</label>
    ...
    <div class="row">
        ...
        ?>
            <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Content</label>
                    <textarea id="mytextarea" name="post" rows="50"><?=$body?></textarea>
                </div>
